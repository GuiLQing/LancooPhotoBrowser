//
//  LancooCustomCameraController.m
...
@implementation UIImage (LancooCustomCamera)
...
- (void)layoutSubviews
{
    ...
    _layoutOK = YES;
    self.tipLabel.frame = CGRectMake(0, -30, CGRectGetWidth(self.bounds), 20);
...
- (void)setupUI
{
    ...
    self.bottomView.backgroundColor = [UIColor colorWithRed:244/255.0 green:244/255.0 blue:244/255.0 alpha:0.9];
    ...
    self.dismissBtn.frame = CGRectMake(60, self.bounds.size.height/2-25/2, 25, 25);
    [self.dismissBtn setImage:LancooPhotoImage(@"lancoo_arrow_down") forState:UIControlStateNormal];
    [self.dismissBtn addTarget:self action:@selector(dismissVC) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.dismissBtn];
    
    self.cancelBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    self.cancelBtn.backgroundColor = [UIColor colorWithRed:244/255.0 green:244/255.0 blue:244/255.0 alpha:0.9];
    [self.cancelBtn setImage:LancooPhotoImage(@"lancoo_retake") forState:UIControlStateNormal];
    ...
    [self.doneBtn setImage:LancooPhotoImage(@"lancoo_takeok") forState:UIControlStateNormal];
...
- (void)showCancelDoneBtn
{
    ...
    doneRect.origin.x = CGRectGetWidth(self.bounds)-doneRect.size.width-40;
...
//--------------------------------------------------------//
//--------------------------------------------------------//
@interface LancooCustomCameraController () <CameraToolViewDelegate, AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate>
{
    //拖拽手势开始的录制
    BOOL _dragStart;
    BOOL _layoutOK;
...
@implementation LancooCustomCameraController
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.sessionPreset = LancooCaptureSessionPreset1280x720;
        self.videoType = LancooExportVideoTypeMp4;
        self.circleProgressColor = [UIColor colorWithRed:80/255.0 green:169/255.0 blue:56/255.0 alpha:1.0];
    }
    return self;
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (_cameraUnavailable) {
        ...
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            if (self.dismissCompletionHandler) self.dismissCompletionHandler();
        ...
        if (!self.allowTakePhoto && !self.allowRecordVideo) {
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:@"allowTakePhoto与allowRecordVideo不能同时为NO" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)viewDidLayoutSubviews
{
    ...
    self.toolView.frame = CGRectMake(0, UIScreen.mainScreen.bounds.size.height-150, UIScreen.mainScreen.bounds.size.width, 100);
    self.previewLayer.frame = self.view.layer.bounds;
    self.toggleCameraBtn.frame = CGRectMake(UIScreen.mainScreen.bounds.size.width-50, UIApplication.sharedApplication.statusBarFrame.size.height, 30, 30);
}
...
- (void)setupUI
{
    ...
    self.focusCursorImageView = [[UIImageView alloc] initWithImage:LancooPhotoImage(@"lancoo_focus")];
    ...
    [self.toggleCameraBtn setImage:LancooPhotoImage(@"lancoo_toggle_camera") forState:UIControlStateNormal];
...
- (NSString *)transformSessionPreset
{
    switch (self.sessionPreset) {
        case LancooCaptureSessionPreset325x288:
            return AVCaptureSessionPreset352x288;
            
        case LancooCaptureSessionPreset640x480:
            return AVCaptureSessionPreset640x480;
            
        case LancooCaptureSessionPreset1280x720:
            return AVCaptureSessionPreset1280x720;
        
        case LancooCaptureSessionPreset1920x1080:
            return AVCaptureSessionPreset1920x1080;
            
        case LancooCaptureSessionPreset3840x2160:
        {
...
- (void)onOkClick
{
    ...
    if (self.doneBlock) self.doneBlock(self.takedImage, self.videoUrl);
    if (self.dismissCompletionHandler) self.dismissCompletionHandler();
    [self dismissViewControllerAnimated:YES completion:nil];
}
...
- (void)onDismiss
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (weakSelf.dismissCompletionHandler) weakSelf.dismissCompletionHandler();
...
- (NSString *)getVideoExportFilePath:(LancooExportVideoType)type
{
    NSString *format = (type == LancooExportVideoTypeMov ? @"mov" : @"mp4");
